...
import { getAttendanceReport, getPendingFundRequests, getBranches } from '../services/api';
...
const DashboardsView = ({ group }) => {
    ...
    const fetchStats = async () => {
        try {
            ...
            // If group name looks like a branch name, show only that
            // If it's a general group or cluster group, show relevant branches
            let targetBranchIds = [];

            // Heuristic: If group object has branchId, use it
            if (group.branchId) {
                ...
                if (matchedBranch) {
                    ...
                    // Fallback: show first few branches for demo or if it's a wide group
            ...
            const stats = await Promise.all(targetBranchIds.map(async (branchId) => {
                ...
                const requests = await getPendingFundRequests('BRANCH_MANAGER');
                const branchRequests = (requests.data || []).filter(r => r.branchId === branchId);

                return {
                    branchName: branch?.name || branchId,
                    presentCount,
                    totalStaff: 25, // Mock total for now
                    pendingRequests: branchRequests.length,
                    todaySales: '₹' + (Math.floor(Math.random() * 50000) + 10000).toLocaleString(), // Mock sales
                    lastSale: 'Just now'
                };
            }));
